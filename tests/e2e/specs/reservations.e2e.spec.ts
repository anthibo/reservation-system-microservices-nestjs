describe('Reservations', () => {
  ...
  beforeAll(async () => {
    const user = {
      email: 'reservationSystemE2E@test.com',
  ...
  test('Create & Get Reservation', async () => {
    const createdReservation = await createReservation();
    const reservationFetched = await fetch(
    ...
    const reservation = await reservationFetched.json();
